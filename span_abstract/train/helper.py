...
def model_save(session, save_path, model_input, model_output):
    ...
    # 定义输入签名
    inputs = {k: tf.saved_model.utils.build_tensor_info(v) for k, v in model_input.items()}

    # 定义输出签名
    outputs = {k: tf.saved_model.utils.build_tensor_info(v) for k, v in model_output.items()}
    ...
    builder.save()
    logging.info("the latest model saved!!")
